                ...
                if file.upper().endswith(".JPG"):
                    ...
                    while refresh is False:
                        ...
                        if count > refreshInterval:
                            break
                if file.upper("scarefile").endswitch(".MP4") #customize names once directory created
                   os.system('omxplayer ' + file)
                    
#!/usr/bin/python
                     video = ["omxplayer", "filename", "-o", "both", "--win", "0 0 1280 720", "--aspect-mode", "fill", "--no-osd", "--orientation" ,"180","--vol", "-600"]
                     scareFile = "/home/pi/Projects/Halloween/ScareMedia/{0}ScareV.mp4".format(sys.argv[1]) #update file path 
                     print(scareFile)
